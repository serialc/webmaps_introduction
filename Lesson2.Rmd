...
- Link to Leaflet CSS
- Link to Leaflet JS
- Link to your CSS from directions
- Link to your JS from directions
...
.pull-right[
Follow the [directions](https://leafletjs.com/SlavaUkraini/examples/quick-start/) for the following sections:
...
As we will not be generating our own tiles in this course we need to use someone's. Mapbox's business is providing tools and resources, such as these tiles.
...
.pull-right[
Many alternative sources exist:

.w75[[![WebMap containing various tile sources](imgs/l2/xyztiles.png)](http://openwhatevermap.xyz)]

Try setting the `tileLayer` to an alternative source, for example:
...
---
class: inverse, center, middle, activity
# Complete your WebMap

---
# An alternative method - QGIS
...
.pr[
Beyond just updating these two maps, you must update your main index.html file that we worked on first today, to point to the two new maps.
]
...
.pl[
1. Finish all the excercises from today's lecture.
2. Create a Leaflet map showing points, markers, and/or polygons, with popups for points of interest in Luxembourg (or another location of your choice that is not London). **Please don't share any personal location data (e.g., house, work).
3. Create a QGIS map of any data/content of your choice.
4. Upload the above two maps to your server space.
5. Both maps must be linked from your main index.html file.
]
