...
## Two HTML files
.pl[
A folder, named `test` has two html files in:
- index.html
- info.html
...
**[Our server for this course: http://167.71.41.80](http://http://167.71.41.80/)**
...
However, this is typically standardized. If want an html file to appear at your URL: `http://167.71.41.80/~cyrille` then you must place your `index.html` file at the location `/home/cyrille/public_html/index.html`.
...
.pl[
We will now create our first WebMap using the [Leaflet library](https://leafletjs.com/).
...
## Replication
Create new WebMaps:
1. *Using Leaflet*, showing points, markers, and/or popups for **points of interest in Luxembourg** (e.g. Good restaurants).
  - .ts[**Please don't share any personal location data (e.g., house, work)**]
2. *Using QGIS* of any theme of your choice.

]

.tl[
## Your server space
1. Upload the above **four maps** to your server space.
...
# activate firewall
ufw enable
ufw status # it show have Apache and OpenSSH enabled
...
# use newusers (for usage see: man newusers) to create users from file with following format:
